...
新增功能：request headers、query_params、SkyResponse（自定义状态码/headers/content-type）、middleware（before_request/after_request）
...
3. **Middleware 开销极小** — GIL 模式含 after_request hook 仅损失约 3%（GET `/`），JSON 路由损失较大（~10%）因为 after_request 需要创建 SkyResponse 对象
