#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║        Pyronova vs Robyn — Head-to-Head Benchmark                   ║
# ╚══════════════════════════════════════════════════════════════════╝
set -e
...
cleanup() {
    lsof -ti:$PYRONOVA_PORT | xargs kill -9 2>/dev/null || true
...
# ─── Create Pyronova bench server ────────────────────────────────
PYRONOVA_SERVER=$(mktemp /tmp/pyronova_vs_XXXXXX.py)
cat > "$PYRONOVA_SERVER" << PYEOF
...
if __name__ == "__main__":
    app.run(host="127.0.0.1", port=$PYRONOVA_PORT, mode="subinterp")
PYEOF
...
declare -a ROUTE_NAMES PYRONOVA_RPS ROBYN_RPS PYRONOVA_P50 ROBYN_P50 PYRONOVA_P99 ROBYN_P99
...
bench_route() {
    ...
    local pyronova_result robyn_result
    pyronova_result=$(run_wrk "http://127.0.0.1:$PYRONOVA_PORT$path" "$DURATION" "$lua_arg")
    robyn_result=$(run_wrk "http://127.0.0.1:$ROBYN_PORT$path" "$DURATION" "$lua_arg")

    ROUTE_NAMES+=("$name")
    PYRONOVA_RPS+=("$(parse_rps "$pyronova_result")")
    ROBYN_RPS+=("$(parse_rps "$robyn_result")")
    PYRONOVA_P50+=("$(parse_latency 50 "$pyronova_result")")
    ROBYN_P50+=("$(parse_latency 50 "$robyn_result")")
    PYRONOVA_P99+=("$(parse_latency 99 "$pyronova_result")")
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║         Pyronova v1.4.0 vs Robyn — Head-to-Head             ║"
echo "╠══════════════════════════════════════════════════════════╣"
printf "║  CPUs: %-47s║\n" "$NUM_CPUS"
...
echo ""
echo "Starting Pyronova on :$PYRONOVA_PORT ..."
python "$PYRONOVA_SERVER" &
PYRONOVA_PID=$!
sleep 2
...
# Verify
curl -sf "http://127.0.0.1:$PYRONOVA_PORT/" > /dev/null || { echo "❌ Pyronova not running"; exit 1; }
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  Results: Pyronova v1.4.0 vs Robyn"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
printf "  %-25s │ %12s │ %12s │ %8s\n" "Route" "Pyronova (req/s)" "Robyn (req/s)" "Ratio"
printf "  %-25s─┼─%12s─┼─%12s─┼─%8s\n" "─────────────────────────" "────────────" "────────────" "────────"
...
echo ""
printf "  %-25s │ %12s │ %12s │\n" "Memory (MB)" "$PYRONOVA_MEM" "$ROBYN_MEM"

echo ""
echo "  Latency P50 / P99:"
printf "  %-25s │ %8s / %-8s │ %8s / %-8s\n" "Route" "Pyronova P50" "P99" "Robyn P50" "P99"
printf "  %-25s─┼─%8s───%-8s─┼─%8s───%-8s\n" "─────────────────────────" "────────" "────────" "────────" "────────"
for i in "${!ROUTE_NAMES[@]}"; do
    printf "  %-25s │ %8s / %-8s │ %8s / %-8s\n" \
        "${ROUTE_NAMES[$i]}" "${PYRONOVA_P50[$i]}" "${PYRONOVA_P99[$i]}" "${ROBYN_P50[$i]}" "${ROBYN_P99[$i]}"
done
