...
Robyn `--fast` 启动 22 个独立 OS 进程（实测 `pgrep -f robyn | wc -l` = 22）：

| 框架 | 进程数 | 空闲 RSS | 负载 RSS |
|------|--------|---------|---------|
...
| 对比维度 | Pyre 优势 |
|---------|----------|
| 空闲内存 | **11.6x** (36.6 vs 424.6) |
| 负载内存 | **6.7x** (67 vs 447) |
| 吞吐量 | **2.7x** (217k vs 81k) |
| 每 MB 吞吐 | **21.5x** (3,240 r/s/MB vs 151 r/s/MB) |

**Pyre 用 1/7 的内存实现了 2.7x 的吞吐量。**
每 MB 内存产出的 QPS 是 Robyn 的 **21.5 倍**。
