...
折扣来源：NUMA 跨域访问（Threadripper 4 NUMA 域，pyronova 目前不做 pinning）+
Tokio scheduler 在 128 threads 的轻微争用 + kernel TCP/epoll 在
百万 req/s 级的 batching 开销。

## 全部实现后的 per-profile 预测

**假设：** #16–#20 全部落地，Pyronova v1.6+，64 核 Threadripper 硬件。

| Profile | Pyronova 预测 req/s | 归一化 (best→100) | + 内存 bonus | 小计 | 现 Actix 分 |
|---|---|---|---|---|---|
...
| Pyronova JSON | 536k @ 8C (`/hello/{name}`) | **67k / core** | **3.6× Actix per-core** |

关键：
- Pyronova 的 JSON 响应走的是 `serde_json + pythonize` —— **全 Rust 路径**。Python 只返回 dict，不碰 json.dumps。
- Actix 自然也是全 Rust 路径 (`serde_json`)。两边 Rust JSON 序列化实现一致。
- 我们单核更快的原因：Pyronova 响应路径更短（sub-interp worker 直返 Rust response），而 Actix 在 64 核上有 NUMA / scheduler 开销。
...
2. **内存 bonus** 基本稳了 —— v1.5.0 实测 0.057 B/req（上限测量噪声），
   ...
   `sqrt(rps)/memoryMB` 给 Pyronova 好分。
